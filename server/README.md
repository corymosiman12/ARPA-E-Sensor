# Intro
This page is meant to describe the process for getting the UV4L library up and running on a Raspberry Pi.  UV4L allows the pi to act as a streaming video server over IP, which can be connected to using `OpenCV` on a client side application.
...
1. Connect the pi to our network `Linksys00926`.
